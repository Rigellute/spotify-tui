...
async fn main() -> Result<(), failure::Error> {
    ...
    let (io_tx, mut io_rx) = mpsc::channel::<IoEvent>(1);
    let mut app = App::new(io_tx);
    let events = event::Events::new(user_config.behavior.tick_rate_milliseconds);
    match start(&mut app, client_config, user_config, events).await {
        Ok(()) => {}
    ...
    //             let mut network = Network::new(&mut oauth, config_port, spotify, token_expiry);
    //             while let Some(io_event) = io_rx.recv().await {
    //                 println!("got = {:?}", io_event);
    //                 network.handle_network_event(io_event, &app).await;
    //             }
    //         });

    //         match start(cloned_app, client_config, user_config, events).await {
    //             Ok(()) => {}
    //             Err(e) => println!("Error {}", e),
    //         };
    //     }
    //     None => {
    //         println!("Spotify failed to authenticate");
    //     }
    // };
...
async fn start(
    ...
    app.io_tx.send(IoEvent::GetPlaylists).await?;
    ...
    loop {
        ...
        if let Ok(size) = terminal.backend().size() {
            ...
            app.small_search_limit = min((f32::from(size.height) / 2.85) as u32, max_limit / 2);
        ...
        {
            // refresh token
            app.io_tx.send(IoEvent::RefreshAuthentication).await?;
        } else {
        ...
        match events.next()? {
            event::Event::Input(key) => {
                ...
                } else if key == app.user_config.clone().keys.back {
